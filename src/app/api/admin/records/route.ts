...
export async function GET(request: Request) {
  try {
    ...
    const result = await getConvexHttpClient(env.CONVEX_URL).query(api.admin.records, { serviceSecret: env.convexServiceSecret, section, paginationOpts: { numItems: 50, cursor: url.searchParams.get("cursor") } });
