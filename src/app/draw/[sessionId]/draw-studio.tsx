...
export default function DrawStudio({ sessionId, tldrawEnabled = false, tldrawLicenseKey }: Props) {
  ...
  return <main className="draw-shell">
    ...
    <section className="canvas-zone" ref={drawingSurfaceRef}><div ref={canvasViewportRef} className="viewport" data-testid="draw-canvas-viewport">{useTldraw && tldrawLicenseKey ? <TldrawAdapter licenseKey={tldrawLicenseKey} onReady={(adapter) => { tldrawAdapterRef.current = adapter; adapter.setBrush(color, size); }} /> : <><canvas ref={backgroundCanvasRef} className={showingGeneratedImage ? "hidden layer" : "layer"} aria-hidden="true" /><canvas ref={strokeCanvasRef} className={showingGeneratedImage ? "hidden layer" : "layer"} onPointerDown={start} onPointerMove={move} onPointerUp={end} onPointerCancel={end} onLostPointerCapture={end} onContextMenu={(event) => event.preventDefault()} aria-label="COAST drawing canvas" data-testid="draw-canvas" /></>}{showingGeneratedImage ? <img className="preview-image" src={job?.outputUrl ?? job?.previewUrl} alt="COAST generated preview" draggable={false} onError={() => setMessage("The image was generated, but its preview could not load. Reopen the card and try again.")} /> : null}<span className="canvas-hint">1024 × 1024</span></div></section>
