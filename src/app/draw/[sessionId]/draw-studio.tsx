...
export default function DrawStudio({ sessionId, tldrawEnabled = false, tldrawLicenseKey }: Props) {
  ...
  useEffect(() => {
    ...
    const startTouch = (event: TouchEvent) => {
      if (canvasViewportRef.current?.contains(event.target as Node) && !(event.target as Element).closest(".canvas-utility-rail")) {
  ...
  return <main className="draw-shell">
    ...
    <style>{`
      ...
      .canvas-workspace{width:min(100%,720px);height:100%;min-height:0;display:flex;align-items:center;justify-content:center}.canvas-utility-rail{position:absolute;top:8px;left:8px;z-index:7;display:flex;gap:5px;padding:4px;border:1px solid #6facf155;border-radius:16px;background:linear-gradient(135deg,#17345ad9,#0a1427e8);box-shadow:inset 0 1px #eff8ff24,0 8px 20px #0005;backdrop-filter:blur(18px) saturate(135%);-webkit-backdrop-filter:blur(18px) saturate(135%)}.canvas-icon-button{position:relative;display:grid;place-items:center;width:44px;min-height:44px;padding:0;border:0;border-radius:11px;background:#1f4d7d8a;color:#dbeeff;box-shadow:inset 0 1px #eff8ff20;cursor:pointer}.canvas-icon-button svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}.canvas-icon-button:disabled,.canvas-icon-button.disabled{opacity:.35;cursor:default}.canvas-icon-button:focus-visible,.canvas-icon-button:focus-within{outline:2px solid #6bc0ff;outline-offset:2px}.import-icon input{position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer}.import-icon.disabled{pointer-events:none}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
      ...
      @media(max-height:680px){.draw-shell{gap:4px}.canvas-utility-rail{top:6px;left:6px;gap:4px;padding:3px}.canvas-icon-button{width:40px;min-height:40px}.ink-controls{min-height:50px;padding-block:4px}.mode-picker button{min-height:46px}}
    `}</style>
