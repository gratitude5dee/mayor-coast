...
export default function DrawStudio({ sessionId, tldrawEnabled = false, tldrawLicenseKey }: Props) {
  ...
  const refreshRevisions = useCallback(async () => {
    ...
    if (restored) {
      ...
      if (restored.outputMediaId) setTab("preview");
    }
  }, [applyJob, sessionId]);
  ...
  function refine() {
    ...
    selectedRevisionRef.current = job.id; setSelectedRevisionId(job.id); setRefining(true); setResetContext(false); setTab("sketch"); setMessage("Result loaded. Add a follow-up instruction or draw a refinement.");
  }
  ...
  function selectRevision(revision: Revision) {
    ...
    if (revision.outputMediaId || revision.previewMediaId) setTab("preview");
  }
